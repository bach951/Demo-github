...
public class updatePlantServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (p == null) {
                request.setAttribute("notValidCateId", "CateID not valid, update Plant fail!, check CateID in Manage categories");
                ArrayList<Plant> listPlants;
                listPlants = PlantDAO.viewPlant();
                request.setAttribute("listPlants", listPlants);
                RequestDispatcher rd = request.getRequestDispatcher("updatePlant.jsp");
                rd.forward(request, response);
            }
            PlantDAO.updatePlant(pName, price, imgPath, description, status, cateId, pid);
            request.setAttribute("message", "update success!");
            ArrayList<Plant> listPlants;
            listPlants = PlantDAO.viewPlant();
            request.setAttribute("listPlants", listPlants);
            RequestDispatcher rd = request.getRequestDispatcher("mainController?action=viewPlantsPage");
            rd.forward(request, response);
        }
    }
